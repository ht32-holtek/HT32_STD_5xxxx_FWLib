...
#if (LIBCFG_PGA_REF_V2)
#define CR_REF_MASK                              (0x3ul << CR_REF_POS)
#endif
...
#define IS_PGA_INPUT(x)                          ((x == PGA_CALIBRATION_INPUT_NEGATIVE) || \
                                                  (x == PGA_CALIBRATION_INPUT_POSITIVE))
...
void PGA_StructInit(PGA_InitTypeDef* PGA_InitStruct)
{
  ...
  #if (LIBCFG_PGA_REF_V2)
  PGA_InitStruct->PGA_REF   = PGA_REF_AGND;
  #else
 ...
 *   This parameter can be one of the following values:
 *     @arg PGA_CALIBRATION_INPUT_NEGATIVE
 *     @arg PGA_CALIBRATION_INPUT_POSITIVE
 * @retval None
*************************************************************************************************************/
...
void PGA_SetModeInverting(HT_PGA0_X_TypeDef* HT_PGAn, PGA_GAIN_TYPE1_Enum PGA_GAIN_x)
{
  ...
  if (HT_PGAn == HT_PGA0)
  {
    PGA_InitStruct.PGA_REF   = PGA_REF_PGA0N;
  }
  else
  {
    PGA_InitStruct.PGA_REF   = PGA_REF_PGA1N;
  }
  #else
...
#if (LIBCFG_PGA_REF_V2)
/*********************************************************************************************************//**
 * @brief Configure the Reference Ground for the selected PGA Control Register.
 * @param HT_PGAn: where HT_PGAn is the selected PGA from the PGA peripherals.
 * @param PGA_REF_x: Where PGA_REF_x is the Reference Ground for PGA registor.
 *   This parameter can be one of the following values:
 *     @arg PGA_REF_AGND
 *     @arg PGA_REF_PGA0N
 *     @arg PGA_REF_PGA1N
 * @retval None
 ************************************************************************************************************/
void PGA_VREFConfig(HT_PGA0_X_TypeDef* HT_PGAn, u8 PGA_REF_x)
{
  u32 CRValue;

  /* Check the parameters                                                                                   */
  Assert_Param(IS_PGA(HT_PGAn));
  Assert_Param(IS_PGA_REF(PGA_REF_x));

  CRValue = HT_PGAn->CR & 0x0000FFFF;
  CRValue &= ~CR_REF_MASK;

  HT_PGAn->CR = gPGAUnProtectKey;
  HT_PGAn->CR = ((u32)PGA_REF_x << CR_REF_POS) | CRValue;
}
#endif
