...
void CMP_Init(HT_CMP_TypeDef* HT_CMPn, CMP_InitTypeDef* CMP_InitStruct)
{
  ...
  #if (LIBCFG_CMP_65x_66x_VER)
  #if (LIBCFG_CMP_POS_INPUT_SEL_V4)
  if (CMP_InitStruct->CMP_InputSelection == CMP_INPUT_PGA0O)
  {
    if (HT_CMPn == HT_CMP0)
    {
      HT_CMPn->CI = 0x00000001;
    }
    else
    {
      HT_CMPn->CI = 0x00000003;
    }
  }
  else
  {
...
void CMP_SetScalerValue(HT_CMP_TypeDef* HT_CMPn, u8 Scaler_Value)
{
  ...
  #if (LIBCFG_CMP_SINGLE_VALR)
  /* The VALR register is shared among CMP                                                                  */
  HT_CMPn = HT_CMP0;
  #endif
